<div class="input-group">
  ...
  <select id="${property}" name="${property}" class="combobox" ${required?'required=true':''}>
    <option value="">${placeholder?:'類別代碼或名稱關鍵字'}</option>
    <g:each in="${com.lch.cces.CertificateCategory.list()}">
  ...
  <g:select name="${property}" value="${value}" required="${required}" class="combobox"
	from="${com.lch.cces.CertificateCategory.list()}"
	noSelection="${['':'類別代碼或名稱關鍵字']}"
...
$(function() {
  $('#${property}').combobox({
    placeholder: "${placeholder?:'類別代碼或名稱關鍵字'}",
    appendId: 'Combo'
  });
});
</asset:script>
