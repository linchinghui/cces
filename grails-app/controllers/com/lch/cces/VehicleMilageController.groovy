...
class VehicleMilageController extends BaseController<VehicleMilage> {
    ...
    private void resolveParameters(params) {
        ...
        if (compIds?.size() >= 1 && compIds[0] != 'null') {
            if (params?.projectId == null)      { params['projectId']      = compIds[0] }
        }
        if (compIds?.size() >= 2 && compIds[1] != 'null') {
            if (params?.dispatchedDate == null) { params['dispatchedDate'] = compIds[1] }
		}
        if (compIds?.size() >= 3 && compIds[2] != 'null') {
            if (params?.vehicleId == null)      { params['vehicleId']      = compIds[2] }
        }

        params.remove('id')

println "after resolve: $params"
    ...
    private List<VehicleMilage> listAllVehicleMilages(Map params) {
        ...
        return VehicleMilage.where {
            ...
            if (params?.dispatchedDate) { dispatchedDate == Date.parse('yyyy/MM/dd', params.dispatchedDate) }
            if (params?.vehicleId     ) { vehicle.id     == params.vehicleId }
        }.list(params)
    }

    protected final List<VehicleMilage> listAllResources(Map params) {
        def milages = listAllVehicleMilages(params)
    }
    ...
    protected final VehicleMilage createResource(Map params) {
        ...
        if (params?.dispatchedDate instanceof String &&
        	params?.dispatchedDate != 'null') {
        	props.dispatchedDate = Date.parse('yyyy/MM/dd', params.dispatchedDate)
        }
        if (params?.vehicleId &&
